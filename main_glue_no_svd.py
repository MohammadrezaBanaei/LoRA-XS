...
class ModelArguments:
    ...
    """
    lora_rank: int = field(
        metadata={"help": "The rank (r) to be used for LoRA."}
    )
...
def main():
    ...
    find_and_initialize(model, peft_config_dict, adapter_name=adapter_name, reconstr_type=reconstr_type,
                        writer=tb_writer, reconstruct_config=reconstr_config)
    ...
    optimizer = torch.optim.AdamW(
        [{"params": [i[1] for i in model.named_parameters() if "classifier" in i[0]],
         ...
         {"params": [i[1] for i in model.named_parameters() if "classifier" not in i[0]],
          "lr": training_args.learning_rate},
         ]
    )

    num_update_steps_per_epoch = math.ceil(len(train_dataset) /
